...
import { addPostMessageIds } from "../../../redux/dialogs/actions";
        ...
        >
          ...
          }
          {
            post.medias.length>0&&(
              <>
                {post.medias[0].type=="video"&&<video controls src={post.medias[0].url} className="chat-post-video" style={getDimension(post.medias[0])}/>}
                {post.medias[0].type=="image"&&<img src={convertChatUrl(post.medias[0].url)} className="chat-post-image" style={getDimension(post.medias[0])}/>}
              </>
            )
          }
        </NavLink>
